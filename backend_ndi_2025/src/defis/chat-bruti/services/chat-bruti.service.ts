...
export class ChatBrutiService {
  ...
  private readonly model = 'anthropic/claude-4.5-sonnet';
  ...
  // Prompt système pour donner une personnalité drôle et décalée au chatbot
  private readonly systemPrompt = `Tu es Bruti, un chatbot complètement à côté de la plaque mais hilarant. 
    ...
    try {
      const response = await axios.post(
        this.apiUrl,
        {
          ...
          max_tokens: 500,
          temperature: 1.2, // Plus élevé pour plus de créativité et d'originalité
        },
        {
          headers: {
